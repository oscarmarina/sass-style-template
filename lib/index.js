...
class SassProcessor {
  ...
  async compile(file) {
    try {
      const result = sass.compile(file, {loadPaths: this.loadPaths});
      return result.css;
    } catch (error) {
...
export class SassStyleTemplate {
   ...
   * @param {string[]} [options.loadPaths=['node_modules']] - Paths to resolve @use/@import.
   * @param {SassProcessor} [options.sassProcessor=new SassProcessor({loadPaths})] - The SASS processor instance.
   ...
   * @param {string[]} files - The resolved file paths to watch.
   */
  watchSass(files) {
    ...
    const watchDirs = [...new Set(files.map((f) => path.dirname(f)))];
    if (!watchDirs.length) {
      return;
    }

    const dirWatcher = chokidar.watch(watchDirs, {ignoreInitial: true, depth: 0});
    ...
    dirWatcher.on('error', (err) => console.error(`Watcher ${color.red}${err}${color.reset}`));
  }
