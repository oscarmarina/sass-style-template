...
class SassStyleTemplate {
  ...
  constructor(
    {
      ...
      template = fs.readFileSync((`${__dirname}/../tpls/sass-template.tmpl`), 'utf8'),
    } = {},
  ) {
    this.options = {
      markerStart, markerEnd, customGlob, cssFile, destination, template,
    };
  ...
  writeTemplate(fileName, cssResult) {
    ...
    console.info(`${chalk.green('Sass Done:')} ${chalk.blue(`${this.fileInfo.fileNameWithoutExt}${this.fileInfo.fileExt}`)}`);
  }
  ...
  renderStylesTemplate(fileName) {
    ...
    this.fileInfo = {
      ...
      fileExt: this.options.cssFile ? '-styles.css' : '-styles.js',
    };
    ...
    if (userFileExists) {
      ...
      if (startReplacePosition >= 0) {
        ...
        content = file.substring(0, startReplacePosition) + cssResult + this.options.markerEnd;
      } else {
        throw Error(`${chalk.red(`No found marker start "${this.options.markerStart}" in file.`)}`);
      }
  ...
  unlinkFile(file) {
    ...
    if (fs.existsSync(fileToUnlink)) {
      try {
        ...
        console.info(`${chalk.red(`file removed ${path.basename(fileToUnlink)}`)}`);
      } catch (err) {
